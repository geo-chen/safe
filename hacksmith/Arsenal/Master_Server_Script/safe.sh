...
echo ''
echo ''
printf 'To run forensics on a sampled pool of servers, press "r" '.
...
echo "(You can use a lower number like 5 for a lower error margin, and 15 for a larger one) " 
read confidenceinterval
echo ''
population=$(wc -l server-list.txt | awk '{print $1}') 
...
sort -R server-list.txt | head -n $numberofmachines > /tmp/iplist.txt
...
cp /tmp/fartifactupload/* /home/$username/Dropbox/hacksmith/Arsenal/splunk/
...
chmod 777 -R /home/$username/Dropbox/hacksmith/Arsenal/splunk/
